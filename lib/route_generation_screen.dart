...
class _RouteGenerationScreenState extends State<RouteGenerationScreen> {
  ...
  Widget _buildArtworkPreview() {
    ...
    final scale = _computeRouteScale(route);
    // 作品制作画面と同じ: 100×100 ボックスに scale を適用した見た目サイズ
    final displaySize = _trajectorySize * scale;

    return Stack(
      children: [
        // グレー背景（作品制作画面のキャンバス背景と同じ）
